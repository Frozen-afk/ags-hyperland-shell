...
// Icon — Gtk.Image with an `icon` prop that accepts an icon name, GIcon
// or file path (the GTK4 Astal.Icon widget does not exist, so this fills
// the same role the gtk3 era one did). Bindings of icon names are passed
// through untouched.
export type IconProps = ConstructProps<Gtk.Image, Gtk.Image.ConstructorProps> & {
    icon?: string | any
    pixelSize?: number
}
export const Icon = ((props: IconProps = {} as any) => {
    const { icon, ...rest } = props as any
    if (icon !== undefined && icon !== null) {
        if (typeof icon === "string") {
            if (icon.startsWith("/") || icon.startsWith("file://")) rest.file = icon
            else rest.iconName = icon
        } else if (icon instanceof Binding) {
            const v = icon.get()
            rest[
                typeof v === "string" && (v.startsWith("/") || v.startsWith("file://"))
                    ? "file"
                    : "iconName"
            ] = icon
        } else {
...
export const Scrollable = astalify<Gtk.ScrolledWindow, Gtk.ScrolledWindow.ConstructorProps>(Gtk.ScrolledWindow, {
    getChildren(self) { return self.child ? [self.child] : [] },
    setChildren(self, children) { self.child = filter(children)[0] ?? null },
})
